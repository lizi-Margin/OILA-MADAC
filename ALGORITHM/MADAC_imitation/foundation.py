...
class ReinforceAlgorithmFoundation(RLAlgorithmBase):
    ...
    def action_making(self, StateRecall):
        ...
        if not AlgorithmConfig.use_bc_update_actor:
            expert_action = None
        else:
            expert_action = np.concatenate([mm["expert_action"] for mm in StateRecall["Latest-Info"]], axis=0)
        ...
        policy_hx = self.policy_hidden_states[active_indices].copy()
        critic_hx = self.critic_hidden_states[active_indices].copy()
        # Reset hidden states for done threads
        for idx in done_indices:
            self.policy_hidden_states[idx].fill(0)
            self.critic_hidden_states[idx].fill(0)
        ...
        traj_framefrag = {
            ...
            "obs":           obs,
            "action":        action,
            # "policy_hx":     policy_hx.copy(),
            # "critic_hx":     critic_hx.copy(),
        }
        if expert_action is not None:
            traj_framefrag.update({'expert_action': expert_action})
