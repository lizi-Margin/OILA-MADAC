...
from uhtk.imitation.utils import safe_dump, safe_dump_traj_pool, safe_load, safe_load_traj_pool
...
class trajectory(TRAJ_BASE):
    ...
    def cut_tail(self):
        ...
        # store trajectory length for sequence-based training
        self.traj_length = p_valid.sum()
        # all done
        return
...
class BatchTrajManager(TrajManagerBase):
    def __init__(self, n_env, traj_limit, trainer_hook):
        ...
        self._save_traj_enabled = AlgorithmConfig.save_traj
        self._save_cnt = 0
    ...
    def clear_traj_pool(self):
        ...
        if self._save_traj_enabled:
            ...
            for traj in self.traj_pool:
                for key in list(traj.__dict__.keys()):
                    setattr(traj, key, None)
            self.traj_pool.clear()
        # self.update_cnt += 1
    ...
    def save_traj(self, finalize=True):
        print(end='\n')
        # Do NOT forget to clear the NaNs in the trajectories, or terible things will happen
        if finalize:
            _, self.traj_pool = self.pool_manager.absorb_finalize_pool(pool=self.traj_pool) 
        safe_dump_traj_pool(self.traj_pool, self._save_cnt, traj_dir=f"{cfg.logdir}/traj_pool/")
        self.traj_pool = []
        self._save_cnt += 1