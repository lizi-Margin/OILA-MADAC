...
class ResidualMLPBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.norm2(out)
        out = out + residual
        return out.clamp_(-CLAMP_VALUE, CLAMP_VALUE)
...
class Net(nn.Module):
    def __init__(self, rawob_shape: tuple, action_spec: Optional[Sequence[int]], **kwargs):
        ...
        if self.img_obs:
            ...
            self.vector_res_blocks = nn.Sequential(
                ResidualMLPBlock(h_dim, hidden_dim=h_dim * 2, dropout=0.05),
                ResidualMLPBlock(h_dim, hidden_dim=h_dim * 2, dropout=0.05),
                ResidualMLPBlock(h_dim, hidden_dim=h_dim * 2, dropout=0.05),
            )
        ...
        self.policy_head = nn.Sequential(
            ResidualMLPBlock(h_dim, hidden_dim=h_dim * 2, dropout=0.1),
            nn.LayerNorm(h_dim),
    ...
    def act(self, obs=None, test_mode=None, avail_act=None,
        ...
        at_feat = at_feat.clamp_(-CLAMP_VALUE, CLAMP_VALUE)
        ...
        ct_bac = ct_bac.clamp_(-CLAMP_VALUE, CLAMP_VALUE)
        ...
        value_input = ct_gru_out + ct_bac
        value_input = value_input.clamp_(-CLAMP_VALUE, CLAMP_VALUE)
