import torch, math, traceback, gc
...
class PPO():
    ...
    def establish_pytorch_graph(self, flag, sample, n):
        ...
        if self.use_bc_update_actor:
            expert_action = _2tensor(sample['expert_action'])
        ...
        # 获取当前策略对专家动作的log概率
        if self.use_bc_update_actor:
            _, expert_actLogProbs, bc_entropy, _, _ = self.policy_and_critic.evaluate_actions(
                obs=obs,
                eval_actions=expert_action,  # 用专家动作作为评估目标
                avail_act=avail_act,
                traj_mask=traj_mask
            )
        #######################################################################
        ...
        ##################################################################################################
        # BC
        # Masked mean for bc_entropy
        bc_loss = 0.0
        if self.use_bc_update_actor:
            masked_distEntropy = bc_entropy * traj_mask.unsqueeze(-1)  # bc_entropy doesn't have last dim
            meanDistEntropy = masked_distEntropy.sum() / batch_agent_size
            masked_actLogProbs = expert_actLogProbs * mask_expanded
            meanActLogProbs = masked_actLogProbs.sum() / batch_agent_size
            policy_loss = cross_entropy_loss = -meanActLogProbs
            entropy_loss = -meanDistEntropy
            # bc_loss = cross_entropy_loss*self.imitation_weight  + dist_entropy_loss
            AT_net_loss = bc_loss = cross_entropy_loss  + entropy_loss * self.entropy_coef  
