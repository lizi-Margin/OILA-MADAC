...
class TrajPoolSampler():
    def __init__(self, n_div, traj_pool, flag, prevent_batchsize_oom=False, mcv=None, use_sequence_sampling=True):
        ...
        if AlgorithmConfig.use_bc_update_actor:
            req_dict =        ['avail_act', 'obs', 'action', 'expert_action', 'actionLogProb', 'return', 'reward', 'value']
            req_dict_rename = ['avail_act', 'obs', 'action', 'expert_action', 'actionLogProb', 'return', 'reward', 'state_value']
        else:
    ...
    def cleanup(self):
        ...
        if hasattr(self, 'traj_mask'):
            del self.traj_mask
        if hasattr(self, 'traj_lengths'):
            del self.traj_lengths
    ...
    def reset_and_get_iter(self):
        ...
        for indices in self.sampler:
            ...
            for key in self.container:
                selected[key] = self.container[key][indices].copy()
