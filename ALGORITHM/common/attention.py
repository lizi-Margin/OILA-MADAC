...
class SimpleAttention(nn.Module):
    ...
    def forward(self, k, q, v, mask=None):
        ...
        ########################## plan B ###########################################################
        # Apply mask to prevent attention to masked positions
        if mask is not None:
            try:
                compat = compat.masked_fill(mask.unsqueeze(-2), -1e9)  # Use -1e9 instead of -inf to prevent NaN
            except:
                compat = compat.masked_fill(mask.unsqueeze(-2), -1e4)  # fp16

        # Compute attention scores
        score = F.softmax(compat, dim=-1)

        # Handle edge case: if all positions are masked, softmax produces NaN
        # Replace NaN with uniform attention over valid positions, or zeros if all masked
        if mask is not None:
            # Where entire rows are masked (all -inf), softmax returns nan
            all_masked = mask.all(dim=-1, keepdim=True)
            score = torch.where(all_masked.unsqueeze(-1), torch.zeros_like(score), score)
        #############################################################################################
