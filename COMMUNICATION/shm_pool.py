...
if not platform.system()=="Linux":  
    ...
    class WinSuperProc(Process):
        def __init__(self, pipe, pipeHelp, index, base_seed, machine_info):
            super(WinSuperProc, self).__init__()
            self.pipe = pipe 
            self.pipeHelp = pipeHelp 
            self.index = index
            self.local_seed = index + base_seed
        ...
        def __del__(self):
            ...
            try:
                kill_process_and_its_children(self)
            except Exception as e:
                print_red('[shm_pool]: 终止进程错误:\n', e)
        ...
        def add_targets(self, new_tarprepare_args):
            for new_target_arg in new_tarprepare_args:
                name, gen_fn, arg = new_target_arg
                if name not in self.target_tracker:
                    self.target_tracker.append(name)
                if arg is None:
                    self.automatic_generation(name, gen_fn)
                elif isinstance(arg, tuple):
                    self.automatic_generation(name, gen_fn, *arg)
        ...
        def execute_target(self, recv_args):
            ...
            for i, recv_arg in enumerate(recv_args):
                ...
                if arg is None:
                    res = getattr(target_obj, dowhat)()
                elif isinstance(arg, tuple):
                    res = getattr(target_obj, dowhat)(*arg)
                else:
                    res = getattr(target_obj, dowhat)(arg)
                res_list[i] = res
            return res_list
        ...
        def run(self):
            ...
            setproctitle.setproctitle(f'HmapShmPoolWorker_{self.index}')
            try:
                while True:
                    # 从主管道接收命令
                    recv_args = self.pipe.recv()
                    if not isinstance(recv_args, list):
                        # 处理控制命令
                        if recv_args == 0:
                            # 从辅助管道接收目标配置
                            targets = self.pipeHelp.recv()
                            self.add_targets(targets)
                        elif recv_args == -1:
                            # 退出信号
                            break
                        else:
                            assert False, f"未知命令: {recv_args}"
                    else:
                        # 执行任务并返回结果
                        result = self.execute_target(recv_args)
                        self.pipe.send(result)
            except KeyboardInterrupt:
                print_red(f'Worker {self.index} 收到中断信号')
            except Exception as e:
                print_red(f'Worker {self.index} 错误:\n{traceback.format_exc()}')
            finally:
                self.__del__()
    ...
    class SmartPool(object):
        def __init__(self, proc_num, fold, base_seed=None):
            ...
            self.proc_pool = [WinSuperProc(pipe=p, pipeHelp=pH, index=cnt, base_seed=self.base_seed, machine_info=GlobalConfig.machine_info)
